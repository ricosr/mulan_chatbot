...
class State:
    ...
    def update_slot_state_value(self, slot_state, slot_value):
    ...
    def get_slot_state_value(self, slot_state):
        return self.state_dict[slot_state]["slot_value"]

    def get_state_confidence(self, slot_state):
    ...
    #                     else:
    #                         self.update_slot_state_value(k, v)
    ...
    def get_current_slot(self, current_slot):
        if self.get_state():
            for slot_key, value_dict in self.get_state().items():
                if self.get_state_confidence(slot_key) == 1:
                    current_slot[slot_key] = value_dict["slot_value"]
                if self.get_state_confidence(slot_key) == 0:
    ...
    def judge_dialogue_state(self):
        if self.state_dict:
            ...
            return True, True   # all slots are OK
        else:
            return False, False